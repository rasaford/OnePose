...
def get_pairswise_distances(pose_files):
    ...
    for i in range(len(pose_files)):
        ...
        seq_name = str(pose_file).split('/')[-3]
...
def covis_from_pose(img_list, pose_paths, covis_pairs_out, num_matched, min_rotation):
    dist, dR, seqs_ids = get_pairswise_distances(pose_paths)
    ...
    with open(covis_pairs_out, 'w') as f:
        f.write('\n'.join(' '.join([str(i), str(j)]) for i, j in pairs))
