...
def gather_3d_ann(kp3d_id_position, kp3d_id_feature, kp3d_id_score, kp3d_id_image, xyzs, points_idxs, feature_dim):
    ...
    for new_point_idx, old_points_idxs in points_idxs.items():
        ...
        scores = []
        for old_point_idx in old_points_idxs:
            descriptors += list(kp3d_id_feature[old_point_idx])
            scores += list(kp3d_id_score[old_point_idx])

        kp3d_position.append(xyzs[new_point_idx])
        kp3d_descriptors += descriptors
        kp3d_scores += scores
        idxs.append(len(descriptors))

    return np.array(kp3d_position), np.array(kp3d_descriptors), np.array(kp3d_scores), np.array(idxs)
...
def get_assign_matrix(xys, xyzs, kp3d_idx_to_kp2d_idx, kp3d_id_mapping):
    ...
    logging.info(f"=> match pairs num: {num_matches}")
    logging.info(f"=> total 2d points: {xys.shape[0]}")
    logging.info(f"=> total 3d points: {xyzs.shape[0]}")
...
def get_kpt_ann(cfg, img_lists, feature_file_path, outputs_dir, points_idxs, xyzs):
    ...
    anno_out_dir = outputs_dir / "anno"
    anno_out_dir.mkdir(exist_ok=True)
